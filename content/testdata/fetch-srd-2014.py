#!/usr/bin/env python3
"""Fetch Open5e's SRD 5.1 data for the armour and items pin-test fixtures in
this directory: srd-2014-armor.json and srd-2014-items.json.

Usage:
    python3 fetch-srd-2014.py armor > srd-2014-armor.json
    python3 fetch-srd-2014.py items > srd-2014-items.json

Open5e's v1 API 403s a request with no User-Agent header; both endpoints get
...
def fetch_armor():
    ...
    for r in sorted(v1, key=lambda r: r["slug"]):
        ...
        weight = f"{int(float(raw_weight))} lb." if raw_weight is not None else None
        rows.append(
            {
                "slug": r["slug"],
                "name": r["name"],
...
def main():
    if len(sys.argv) != 2 or sys.argv[1] not in ("armor", "items"):
        print("usage: fetch-srd-2014.py armor|items", file=sys.stderr)
        sys.exit(1)
    rows = fetch_armor() if sys.argv[1] == "armor" else fetch_items()
