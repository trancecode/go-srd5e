...
// SRD's bundle (see srdBundleSize).
type srdItem struct {
	Key      string `json:"key"`
	Name     string `json:"name"`
	Category string `json:"category"`
...
// srdBundleSize is the count Open5e's per-unit cost and weight must be
// scaled by to reach the bundle the SRD 5.1 gear table prices ammunition
// in; a Gear() id absent from this map (every non-ammunition entry) scales
// by 1, matching Open5e's per-unit pricing directly.
var srdBundleSize = map[string]int{
	"arrow":          20, // SRD 5.1 "Arrows (20)"
	"blowgun-needle": 50, // SRD 5.1 "Blowgun needles (50)"
	"crossbow-bolt":  20, // SRD 5.1 "Crossbow bolts (20)"
	"sling-bullet":   20, // SRD 5.1 "Sling bullets (20)"
}
...
// srdGearCostWeight returns the cost and weight Gear()'s entry id must
// carry to match the SRD 5.1 text: raw.Cost/raw.Weight scaled by
// srdBundleSize, then corrected per srdItemCorrections. It fails t if a
// correction's recorded "was" value no longer matches the scaled figure,
// since that means the correction has become obsolete.
func srdGearCostWeight(t *testing.T, id string, raw srdItem) (core.Coins, core.Weight) {
	t.Helper()
	bundle := srdBundleSize[id]
	if bundle == 0 {
		bundle = 1
	}
...
func TestGearMatchesTheSrd(t *testing.T) {
	...
	for id, key := range gearSrdKey {
		...
		wantCost, wantWeight := srdGearCostWeight(t, id, raw)
