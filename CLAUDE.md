...
**Build order and status (dependency DAG):** `core` (done) → `dice` (done) → `check` + `combat` (done) → `damage` (done) → `effect` (done) → `turn` (done) → `resource` (done) → `content`. Each package is a separate plan; build the next one only after the prior is merged and green. Tag `v1.0.0` once all packages are implemented (the repo is public; pkg.go.dev publishes the godoc off the tag).
